...
> ⚠️ **重要：每次部署后必须手动补绑 `db` / `kv`**
>
> 本仓库的 `wrangler.toml` **不声明** D1/KV 绑定（避免把真实资源 ID 写进公开仓库，且
> Wrangler 不支持对 `database_id`/KV `id` 做 `${}` 环境变量插值）。`wrangler deploy`
> 会用本地配置覆盖远端，**凡未声明的绑定都会被清空**——因此**每次触发 Cloudflare
> 部署后，`db` 与 `kv` 两个绑定会丢失，需回到 Worker > Settings > Bindings 重新绑上**，
> 然后再执行初始化（见第 5 步）。`[ai]`/`[assets]` 无需 ID、始终保留，不受影响。

## 5. 初始化数据库

> 每次手动补绑 `db`/`kv` 后建议执行一次，确保表结构是最新的。`/api/init` 是**幂等**的：
> 内部使用 `CREATE TABLE IF NOT EXISTS` 与 `ADD COLUMN`（重复列会自动跳过 try/catch），
> **不会清空既有数据**，可安全反复执行。

1. 确认 `db`、`kv` 绑定已生效（见第 4 步）。
2. 访问（返回 `success` 即成功）：
...
3. `<jwt_secret>` 必须与你配置的 `jwt_secret` 变量一致（见第 3 步）。
